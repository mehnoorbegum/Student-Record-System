     from tkinter import *
      ...
      import subprocess
     def login():
          if usernameEntry.get() == '' or passwordEntry.get() == '':                 messagebox.showerror('Error', 'Fields cannot be empty')
    elif usernameEntry.get() == 'Team16' and passwordEntry.get() == '1234':
        messagebox.showinfo('Success', 'Welcome')
        window.destroy()
        # Start the 'sms.py' and 'acad.py' files in separate processes
        process_sms = multiprocessing.Process(target=run_sms)
        process_acad = multiprocessing.Process(target=run_acad)
        process_sms.start()
        process_acad.start()
    else:
...
def run_sms():
    subprocess.call(['C:/Users/abdul kareem/PycharmProjects/lubna/venv/Scripts/python.exe', 'sms.py'])

def run_acad():
    subprocess.call(['C:/Users/abdul kareem/PycharmProjects/lubna/venv/Scripts/python.exe', 'acad.py'])
...
if _name_ == '_main_':
    ...
    usernameLabel = Label(loginFrame, image=usernameImage, text='username', compound=LEFT, font=('times new roman', 20, 'bold'), bd='5', fg='royalblue')
    usernameLabel.grid(row=1, column=0, pady=10, padx=20)

    usernameEntry = Entry(loginFrame, font=('times new roman', 20, 'bold'), bd='5', fg='maroon')
    usernameEntry.grid(row=1, column=1, pady=10, padx=20)

    passwordImage = PhotoImage(file='password.png')
    passwordLabel = Label(loginFrame, image=passwordImage, text='Password', compound=LEFT, font=('times new roman', 20, 'bold'), bd='5', fg='royalblue')
    passwordLabel.grid(row=2, column=0, pady=10, padx=20)

    passwordEntry = Entry(loginFrame, font=('times new roman', 20, 'bold'), bd='5', fg='maroon',show='*')
    passwordEntry.grid(row=2, column=1, pady=10, padx=20)

    loginButton = Button(loginFrame, text='Login', font=('times new roman', 18, 'bold'), width=15, fg='white',
                         bg='cornflowerblue', activebackground='black', activeforeground='pink', cursor='hand2',
                         command=login)
...
root.resizable(0,0)
root.title('Student Record System(personal info)')

datetimeLabel=Label(root,font=('times new roman',18,'bold'))
datetimeLabel.place(x=5,y=5)
clock()
s='Student Record System(personal info)' #s[count]=t when count is 1
